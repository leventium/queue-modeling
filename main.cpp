...
int main()
{
	...
	while (flag)
	{
		...
		for (int i = 1; i < kass; i++)
			if (timers[i] < min && timers[i] != 0)
				min = timers[i];
		for (int i = 0; i < min; i++)
		{
			nowtime++;
			for (int j = 0; j < kass; j++)
				timers[j]--;
			Sleep(1000);
			system("cls");
			printQueue(nqueue, queue, K, nowtime, kass, timers);
		...
		if (!toend && !(d % 5))
		{
			...
			min = 1;
			cout << "> Добавить в очередь." << endl
				 << "  Вывести статистику." << endl
...
int read(void)
{
	...
	do
	{
		...
		if (!k)
		{
			if (strcmp(str, "0") == 0)
				n = 0, f = 0;
			else if (strcmp(str, "") != 0 && !strchr(str, ','))
			{
				char *s = str;
				while (isdigit(*s))
					s++;
...
void makeStat(int kasses, int *times, int *nqueue, int queue[][30], int *nstat, int stat[][70], int K[][5], int *count, int *flag, int *d)
{
	...
	else
	{
		...
		*count += min;
		if (K[1][0] <= 0 && K[1][1] <= 0 && K[1][2] <= 0 && K[1][3] <= 0 && K[1][4] <= 0)
			*flag = 0;
	}
}
